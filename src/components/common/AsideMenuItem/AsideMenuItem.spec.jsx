...
describe('<AsideMenuItem />', () => {
  it('Should render the AsideMenuItem correctly', () => {
    ...
    expect(screen.getByRole('heading', { text: mock.label })).toBeInTheDocument();

    // Did are the class applied?
    expect(screen.getByRole('listitem')).toHaveAttribute('class', 'aside-menu-item cursor-pointer');
  });
  ...
  it("Should add 'active' to the class when the prop 'active' is set to 'true'", () => {
    ...
    // Did are the class applied?
    expect(screen.getByRole('listitem')).toHaveAttribute('class', 'aside-menu-item cursor-pointer active');
  });
});
